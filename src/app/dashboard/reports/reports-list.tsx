...
type ReportRow = ScoutingEntry & {
  ...
  matches: {
    ...
    event_id: string;
    events: { name: string; year: number; tba_key: string } | null;
  } | null;
};
...
export function ReportsList({
  ...
  // profiles/matches data — so we use the initial server data for display
  // and only use realtime for detecting inserts/deletes.
  const { entries, newIds, clearNew } = useRealtimeScoutingEntries(
    initialReports,
    orgId
  );
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      for (const id of newIds) clearNew(id);
    }, 5000);
    return () => clearTimeout(timeout);
  }, [newIds, clearNew]);

  // Cast back — the realtime hook works with the base type, but our
  // initial data carries the joined relations. New entries from realtime
  // won't have joins, so we handle that gracefully below.
  const reports = entries as ReportRow[];
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            return (
              <div
                key={report.id}
                className={`rounded-2xl dashboard-panel dashboard-card p-5 transition-all duration-500 ${
                  isNew ? "ring-2 ring-teal-400/60" : ""
                }`}
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    {isNew && (
                      <span className="ml-2 rounded-full bg-teal-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-teal-300 animate-pulse">
                        Live
                      </span>
                    )}
                  </div>
